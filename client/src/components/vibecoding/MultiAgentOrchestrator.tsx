...
export function MultiAgentOrchestrator() {
  ...
  return (
    <div className="w-full h-full flex flex-col">
      {/* Header bar */}
      <div className="bg-gradient-to-r from-indigo-500/10 to-purple-500/10 border-b px-6 py-4">
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="w-64 border-r bg-muted/10 p-4">
          ...
          <div className="mt-8 space-y-4">
            <h3 className="text-sm font-semibold text-muted-foreground">Statistikk</h3>
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-sm">Aktive agenter</span>
                <span className="font-bold">{agents.filter(a => a.status !== 'idle').length}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm">Aktive forslag</span>
                <span className="font-bold">{proposals.filter(p => p.status === 'pending' || p.status === 'voting').length}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm">Godkjente</span>
                <span className="font-bold">{proposals.filter(p => p.status === 'approved').length}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm">Oppgaver</span>
        ...
        <div className="flex-1 overflow-auto p-6">
          {activeTab === 'overview' && (
            <div className="space-y-6">
              {/* Agent Grid - now horizontal */}
              <div className="flex gap-4 overflow-x-auto pb-4">
                {agents.map(agent => (
                  <Card key={agent.id} className="relative overflow-hidden min-w-[250px]">
